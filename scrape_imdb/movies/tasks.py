...
def scrape_function():
    ...
    response = requests.get('https://www.imdb.com/chart/top', headers=headers, params=params)
    ...
    for rank, movie_tag in enumerate(movies_tags):
        movie_image = movie_tag.find("td", class_ = "posterColumn").find("img").attrs['src']
        ...
        movie_rating_text = movie_tag.find("td", class_ = re.compile("imdbRating")).find("strong").get_text().strip()
        movie_rating = float(movie_rating_text)
        movie_ranking = rank + 1

        movie , created = Movie.objects.update_or_create(title = movie_title, defaults = {
                    "title" : movie_title,
                    "link" : movie_link,
                    "rating" : movie_rating,
                    "rank" : movie_ranking,
